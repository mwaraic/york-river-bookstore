...
class FilteredListView(ListView):
    ...
    def get_queryset(self):
        ...
        if (self.kwargs.get('cat') == 'All'):
            queryset = YrbOffer.objects.select_related(
                'title__cat').filter(club=self.kwargs.get('club'))
        else:
            queryset = YrbOffer.objects.select_related('title__cat').filter(
                title__cat=self.kwargs.get('cat').lower(), club=self.kwargs.get('club'))
